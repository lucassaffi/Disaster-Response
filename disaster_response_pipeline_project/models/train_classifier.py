...
def load_data(database_filepath):
    ...
    Input: Path of saved sql database
    Output: Dataframe X (messages), Dataframe Y (categories), Y column names
...
def tokenize(text):
    ...
    Input: Message
    Output: List of strings processed
...
def build_model(X):
    ...
    Output: Gridsearch model with pipeline as a parameter
...
def evaluate_model(model, X_test, Y_test, category_names):
    """
    Evaluate the model based on precision, recall and f1_score
    
    Input: Gridsearch model, data for testing (X and Y), category names
    Output: Print results of the model
...
def save_model(model, model_filepath):
    """
    Save model to be access by the app
    
    Input: model, path to save the model
    Output: model saved 
